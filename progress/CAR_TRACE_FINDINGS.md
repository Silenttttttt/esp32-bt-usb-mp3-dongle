...
| 31 | Second-device pairing test, 13:49-13:53 (phone BT off, laptop pairs "Golzin" via `sim/bt_pair_laptop.py`, BlueZ D-Bus) | Classic is discoverable and accepts the ACL link (Connected=true), but **pairing fails every time**: `AuthenticationTimeout` after ~31 s (2 real attempts; the first 2 script runs deadlocked on the agent, script bug, fixed). A2DP Connect without a bond: `br-connection-key-missing`. The classic **rebooted during the 1st real attempt** (13:52, ~930 UART breaks) but **not** the 2nd (13:53) -> the reboot isn't reproducible; the pairing failure is. Next: Muni retries from the laptop's GUI; classic-side log needed (S3 could echo the classic's 'C' frames). |
| 32 | GNOME GUI pairing, 13:55 | Classic **rebooted again** during pairing (brk 3768 -> 4700) -> 2 of 3 real attempts crashed it. GUI then showed "Please enter the following PIN on Golzin: 303657" and hung -- BlueZ chose **legacy PIN / passkey entry**, i.e. the classic isn't doing SSP Just Works as its comments assume (IO_CAP_NONE). The classic can't enter a PIN -> timeout. Explains the script's AuthenticationTimeout. Fix at home on the bench: the classic's pairing mode (SSP / IO caps / PIN handling) + the crash during pairing. |
| 33 | Pairing, lead for the bench | BlueZ reports Golzin `LegacyPairing=true` (no SSP). Muni thinks the PIN dialog doesn't matter. To check: the library's `ESP_BT_GAP_PIN_REQ_EVT` handler (BluetoothA2DPSink.cpp:446) only stores the address and never replies with a PIN, and `self_healing_gap_callback` only answers CFM_REQ -- so a legacy PIN request from a new device may go unanswered. Phones already bonded skip it. |
