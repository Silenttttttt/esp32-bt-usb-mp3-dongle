// PC-hosted stand-in for the FAT16 FALLBACK firmware
// (esp32-s3-msc-fat16-fallback/esp32-s3-msc-fat16-fallback.ino) -- a
// near-identical twin of s3_real_firmware_host.cpp (the FAT12 primary's
// stand-in), included here for the fallback so it gets the same live,
// real-classic-ESP32-backed validation rigor instead of staying
// compiled-but-never-run. This is NOT a separate simulator -- it
// #includes the exact same fat16_disk_shared.h the real fallback .ino
// compiles from
...
#include "../esp32-s3-msc-fat16-fallback/fat16_disk_shared.h"
#include "../esp32-s3-msc-fat16-fallback/silence_primer.h"
...
static int g_radio_port = 9402;  // different from the FAT12 primary's 9401
  ...
  while (true) {
    ...
    if (!resolve_serial_port(expected_serial, path, sizeof(path))) {
      if (attempt == 1 || attempt % 10 == 0) {
        fprintf(stderr, "[s3-host-fat16] board (serial %s) not currently found on any "
                         "/dev/ttyACM* -- waiting...\n", expected_serial);
      }
    ...
    if (fd < 0) {
      if (attempt <= 10 || attempt % 10 == 0) {
        fprintf(stderr, "[s3-host-fat16] open %s failed (attempt %d): %s -- retrying...\n",
    ...
    fprintf(stderr, "[s3-host-fat16] opened %s (serial %s) at %d baud (attempt %d)\n",
...
static void receive_from_esp32(int fd) {
  ...
  while (true) {
    ...
    // events from a real phone become visible here).
      ...
      text[length] = 0;
      fprintf(stderr, "[s3-host-fat16][CONTROL] %s\n", text);
    }

    auto now = std::chrono::steady_clock::now();
    if (std::chrono::duration<double>(now - last_log).count() >= 1.0) {
      fprintf(stderr, "[s3-host-fat16][serial-rx] t=%.2fs audio_received=%lluB\n",
...
static void link_supervisor() {
  while (true) {
    ...
    try {
      ...
      fprintf(stderr, "[s3-host-fat16][RECONNECTING] serial ingestion stopped -- "
...
static void serve_radio(int conn) {
  ...
  while (true) {
    ...
    auto now = std::chrono::steady_clock::now();
    if (std::chrono::duration<double>(now - last_log).count() >= 1.0) {
      fprintf(stderr, "[s3-host-fat16][reads] t=%.2fs reads=%llu straddled(zero-fill-gap)=%llu "
                       "lock_miss=%llu\n",
  ...
  fprintf(stderr, "[s3-host-fat16] radio disconnected (%llu reads served, %llu straddled, "
                   "%llu lock_miss)\n",
          (unsigned long long)read_count, (unsigned long long)straddle_count,
          (unsigned long long)lock_miss_count);
}
...
static void report_resource_usage() {
  ...
  while (true) {
    ...
    fprintf(stderr,
        "[s3-host-fat16][RESOURCE] real-target-comparable: ring_buffer=%.1fKB "
    ...
    if (static_cache_bytes > S3_SRAM_BUDGET_BYTES) {
      fprintf(stderr, "[s3-host-fat16][RESOURCE][WARNING] static caches alone "
                       "EXCEED the real S3's total SRAM budget -- this "
                       "would not fit on real hardware!\n");
    }
    if (ring_bytes > S3_PSRAM_BUDGET_BYTES) {
      fprintf(stderr, "[s3-host-fat16][RESOURCE][WARNING] ring buffer EXCEEDS "
...
int main(int argc, char **argv) {
  ...
  if (!g_ring) {
    fprintf(stderr, "[s3-host-fat16] FATAL: allocation for ring buffer failed\n");
    return 1;
  }
  memset(g_ring, 0, DECLARED_FILE_SIZE);
  disk_append(SILENCE_PRIMER, SILENCE_PRIMER_LEN, false);

  fprintf(stderr, "[s3-host-fat16] running the REAL FAT16 FALLBACK firmware's disk "
                   "logic (fat16_disk_shared.h) on this PC -- validation stand-in, "
                   "not the primary FAT12 path\n");
  fprintf(stderr, "[s3-host-fat16] FAT16 volume: %u sectors, declared file size %u bytes, "
  ...
  if (bind(srv, (struct sockaddr *)&addr, sizeof(addr)) != 0) {
    fprintf(stderr, "[s3-host-fat16] FATAL: bind on port %d failed: %s\n", g_radio_port, strerror(errno));
    return 1;
  }
  listen(srv, 1);
  fprintf(stderr, "[s3-host-fat16] waiting for radio on port %d...\n", g_radio_port);

  while (true) {
    int conn = accept(srv, nullptr, nullptr);
    if (conn < 0) continue;
    fprintf(stderr, "[s3-host-fat16] radio connected\n");
    serve_radio(conn);
    close(conn);
  }
}
