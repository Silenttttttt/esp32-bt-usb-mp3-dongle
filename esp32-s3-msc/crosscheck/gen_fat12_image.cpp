// Generates a complete, byte-for-byte-accurate disk image matching what
// the real S3 firmware's boot sector/FAT/root-dir/data-region logic
// would present over USB-MSC, for real verification against Linux's own
// standards-compliant vfat driver (not just our own from-scratch logic).
#include <cstdio>
#include <cstdint>
#include <cstring>
using namespace std;
...
int main(int argc, char **argv) {
  const char *out_path = argc > 1 ? argv[1] : "fat12_image.bin";
  ...
  fprintf(stderr, "wrote %s: %u total sectors, %u bytes, declared file size %u\n",
